...
class Database(private val file: String = "data/taskmanager.db") {
    ...
    // In Database.kt, replace the deleteUser function:
    fun deleteUser(uid: Long) =
        conn.prepareStatement("UPDATE users SET is_active = 'false' WHERE id = ?").use { s ->
            s.setLong(1, uid)
            s.executeUpdate() == 1
        }
    ...
    fun find(u: String) =
        conn.prepareStatement("SELECT * FROM users WHERE (username=? OR email=?) AND is_active='true'").use { s ->
            s.setString(1, u)
            s.setString(2, u.lowercase())
            s.executeQuery().use { if (it.next()) it.toU() else null }
        }
    private fun ResultSet.toU() = User(
        ...
        getString("username"),
        getString("email"),
        getString("is_active") == "true" // Map 1 to true, 0 to false
    )
